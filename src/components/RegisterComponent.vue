    ...
    // Walidacja hasła
      ...
      try {
        ...
        if (response.data.success) {
          // Obsługa sukcesu - przekierowanie do logowania
...
.register-container input[type="password"] {
  ...
  border-radius: 10px; /* zaokrąglenie do pól formularza */
  padding: 4px;
...
.form-group {
  display: flex; /* Ustawienie Flexbox dla grupy formularza */
  align-items: center; /* Wyrównanie elementów w pionie */
  margin-bottom: 15px; /* Odstęp między grupami formularza */
}

.form-group label {
  width: 20%; /* Szerokość etykiety */
  text-align: right; /* Wyśrodkowanie tekstu etykiety do prawej */
  margin-right: 10px; /* Odstęp między etykietą a polem tekstowym */
  font-size: 17px;
}
...
.form-group input[type="password"] {
  flex-grow: 1; /* Pozwala polu tekstowemu rozciągać się i wypełniać dostępną przestrzeń */
  margin-left: 10px;
  border-radius: 10px;
  padding: 4px;
  border: 2px solid #0e4928;
